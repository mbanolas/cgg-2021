    ...
    public function getTablaTalleresListas(){
       ...
       // foreach($lista as $k=>$v){
        for($k=0;$k<$medio;$k++){    
            ...
            if($lista[$k]['tipo_taller']=='Professional') $class="class='profesional'"; else $class=""; 
            ...
            if(isset($lista[$k+$medio]['id'])){
                ...
                if($lista[$k+$medio]['tipo_taller']=='Professional') $class="class='profesional'"; else $class=""; 
            ...
            if(isset($lista[$k+$medio*2]['id'])){
                ...
                if($lista[$k+$medio*2]['tipo_taller']=='Professional') $class="class='profesional'"; else $class=""; 
            ...
            $tabla.='<td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
             if(isset($lista[$k+$medio*3]['id'])){
                ...
                if($lista[$k+$medio*3]['tipo_taller']=='Professional') $class="class='profesional'"; else $class=""; 
    ...
    public function pdfAsistentes()
  {
   ...
   if($tipoInforme==1) $this->pdf->Cell(25,7,'PAGADO EUR','TBRL',0,'L','1');
    ...
    foreach ($asistentes as $k=>$asistente) {
      ...
      if($tipoInforme==1) $this->pdf->Cell(25,5,$asistente['pagado']==0?'':$asistente['pagado'].'   ','BLR',0,'R',0);
   ...
   return ($tex);
    ...
    foreach($CNombres as $k=>$v){
        ...
        $this->pdf->Cell(15,8,iconv('UTF-8', 'CP1252',number_format($importes[$k],2).' €'),$m,0,'R');
    ...
    if(isset($porciones[0])) $porciones[0]=substr($porciones[0],0,5); else $porciones[0]="";
    if(isset($porciones[1])) $porciones[1]=substr($porciones[1],0,5); else $porciones[1]="";
    ...
    foreach($CNombres as $k=>$v){
        ...
        $id=$id_talleres[$k];
        $v=$this->db->query("SELECT nombre_corto FROM casal_talleres WHERE id='$id'")->row()->nombre_corto;
         $nombre=$v. " - (".$trimestres[$k].")  ".$importes[$k]." €"; 
    ...
    foreach($CNombres as $k=>$v){
        ...
        $this->pdf->Cell(15,8,$trimestres[$k],$m,0,'L');
        $this->pdf->Cell(15,8,iconv('UTF-8', 'CP1252',-$importes[$k].' €'),$m,0,'R');
