<div class="container"><br />
    <h3>Llistat inscripcios a tallers</h3>
    <br />

    <?php echo form_open('talleres/pdfAsistentes', array('class' => "form-horizontal", 'role' => "form")); ?>
    ...
    <div class="col-sm-12">
        <div class="form-group">
            ...
            <div class="col-sm-2 hide orden">
                <?php echo form_radio('orden', "ordenNombre", true, array('class' => '', 'id' => 'ordenNombre')); ?> Orden por Nombre
            </div>
            <div class="col-sm-2 hide orden">
                <?php echo form_radio('orden', "ordenNumSocio", false, array('class' => '', 'id' => 'ordenNumSocio')); ?> Orden por Núm Socio
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-2">
        <button type="submit" class="btn btn-success hide" id="pdfAsistentesSinTelefono" name="pdfAsistentesSinTelefono">
            <span class="glyphicon glyphicon-save-file" aria-hidden="true"></span>
            PDF SENSE telèfon</button>
    </div>
    <div class="col-sm-2">
        <button type="submit" class="btn btn-success hide" id="pdfAsistentesConEmail" name="pdfAsistentesConEmail">
            <span class="glyphicon glyphicon-save-file" aria-hidden="true"></span>
            PDF AMB email</button>
    </div>
    <div class="col-sm-2">
        <button type="submit" class="btn btn-success hide" id="pdfAsistentesSin" name="pdfAsistentesSin">
            <span class="glyphicon glyphicon-save-file" aria-hidden="true"></span>
            PDF SENSE imports</button>
    </div>
    <div class="col-sm-2">
        <button type="submit" class="btn btn-success hide" id="pdfAsistentesCon" name="pdfAsistentesCon">
            <span class="glyphicon glyphicon-save-file" aria-hidden="true"></span>
            PDF AMB imports</button>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#buscarTaller').keyup(function() {
            ...
            setTimeout(
                function() {
                    ...
                    ponerTableAsistentesTaller(taller, orden)
                }, 1000);
        ...
        $('input[name="periodo[]"]').click(function() {
            $('#pdfAsistentesSin').addClass('hide')
            $('#pdfAsistentesCon').addClass('hide')
            $('#pdfAsistentesSinTelefono').addClass('hide')
            $('#pdfAsistentesConEmail').addClass('hide')
            $('#tablaAsistentes').html("");
            $('.orden').addClass('hide')
            $('#talleres').val(0)
        ...
        //cargar todos los cursos de curso
        cursos()
        ...
        function ponerTableAsistentesTaller(taller, orden) {
            //alert(periodo)
            if (taller == 0) {
                $('#tablaAsistentes').html("");
                return false;
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url() ?>" + "index.php/talleres/getTablaAsistentesTaller",
                data: {
                    taller: taller,
                    periodo: periodo,
                    orden: orden
                ...
                error: function() {
                    alert("Error en el proceso getTablaAsistentesTaller. Informar");
                }
        ...
        $('select#talleres').change(function() {
            ...
            //inicialización
            $('#pdfAsistentesSin').removeClass('hide')
            $('#pdfAsistentesCon').removeClass('hide')
            $('#pdfAsistentesSinTelefono').removeClass('hide')
            $('#pdfAsistentesConEmail').removeClass('hide')
            $('#tablaAsistentes').html("");
