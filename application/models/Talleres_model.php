...
class Talleres_model extends CI_Model
{
    ...
    function getTalleres($curso, $periodo = 'C', $tipoTaller = "")
    {
        ...
        switch ($numPeriodo) {
            case 1:
                $idPeriodo = " AND (id_periodo=1 OR id_periodo=3 OR id_periodo=5 OR id_periodo=7) ";
                break;
            case 2:
                $idPeriodo = " AND (id_periodo=2 OR id_periodo=3 OR id_periodo=6 OR id_periodo=7) ";
                break;
            case 4:
                $idPeriodo = " AND (id_periodo=4 OR id_periodo=5 OR id_periodo=6 OR id_periodo=7) ";
                break;
            case 7:
                $idPeriodo = "  ";
                break;
    ...
    function registrarBajas()
    {
        ...
        foreach ($resultNuevo as $k => $v) {
            ...
            if (
                $v['trimestres'] != $resultActual[$k]['trimestres'] ||
    ...
    function registrarBajasNuevo()
    {
        ...
        foreach ($resultNuevo as $k => $v) {
            ...
            if ($v['pagado'] != 0) {
                switch ($periodos) {
                    case 1:
                        $formaPago[] = $this->formaPago($socio, $v['id_taller'], 1);
                        break;
                    case 2:
                        $formaPago[] = $this->formaPago($socio, $v['id_taller'], 2);
                        break;
                    case 4:
                        $formaPago[] = $this->formaPago($socio, $v['id_taller'], 4);
                        break;
                    case 3:
                        $formaPago[] = $this->formaPago($socio, $v['id_taller'], 1);
                        $formaPago[] = $this->formaPago($socio, $v['id_taller'], 2);
            ...
            // mensaje('socio '.$socio);
            // mensaje('id_taller '.$v['id_taller']);
            // mensaje('periodo '.$v['trimestres']);
            // mensaje('importe '.$v['pagado']);
        }
        $formaPago = array_unique($formaPago);
        switch (count($formaPago)) {
            case 1:
                $formaPagoResultado = $formaPago[0];
                break;
            case 0:
                $formaPagoResultado = "metalico";
                break;
            default:
                $formaPagoResultado = "mixto";
        }
        // if(count($formaPago)==1) $formaPagoResultado=$formaPago[0];
        // else $formaPagoResultado="mixto";
        mensaje('forma de pago ' . $formaPagoResultado);
        //obtenemos talleres en espera
        if (isset($Espera))
            foreach ($Espera as $k => $v) {
                $sql = "SELECT t.nombre_corto as nombreTaller, t.id as id_taller
        ...
        $this->load->model('socios_model');
        $tiposPago = array();
        foreach ($id_talleres as $k => $v) {
            mensaje($socio);
            mensaje($id_talleres[$k]);
            mensaje($trimestres[$k]);
        ...
        $formaPagoCatalan = "";
        switch ($formaPagoResultado) {
            case "metalico":
                $formaPagoCatalan = "Metàl·lic";
                break;
            case "tarjeta":
                $formaPagoCatalan = "Targeta";
                break;
            case "mixto":
                $formaPagoCatalan = "";
                break;
    ...
    function formaPago($socio, $id_taller, $periodo)
    {
        mensaje('&&&&&&&&&&&&& paso por formaPago');
        $sql = "SELECT metalico,tarjeta FROM casal_lineas_recibos 
                            WHERE id_socio='$socio' AND id_taller='$id_taller'  AND periodos='$periodo' ORDER BY id DESC LIMIT 1";
        $row = $this->db->query($sql)->row();
        if ($row->metalico != 0 && $row->tarjeta == 0) return "metalico";
        if ($row->tarjeta != 0 && $row->metalico == 0) return "tarjeta";
        if ($row->tarjeta != 0 && $row->metalico != 0) return "mixto";
    }
    ...
    function registrarLineasRecibo(
        ...
        mensaje('paso por registrarLineasRecibo ' . $importe . ' ' . $tarjeta . ' ' . $metalico);
